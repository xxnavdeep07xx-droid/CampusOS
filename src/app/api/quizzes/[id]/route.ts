 ...
 *
 * Body: { title?, description?, dueDate?, isPublished? }
 *
 * Used by the teacher to edit a quiz's metadata (title/description/due date
 * /publish state). Question-level edits go through the quiz builder, not here.
 *
...
export async function PATCH(
  ...
  if (!cls || cls.teacher_id !== user.id) {
    return NextResponse.json(
      { error: "Only the teacher can edit this quiz." },
      { status: 403 }
    );
  }
  ...
  if (updateErr || !updated) {
    return NextResponse.json(
      { error: updateErr?.message ?? "Could not update quiz." },
      { status: 500 }
    );
  }

  return NextResponse.json({ quiz: updated as Quiz });
}
